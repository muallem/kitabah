...
<ul class="navbar-nav ms-auto">
    @foreach($thesis as $item)
        <li class="nav-item sidebar2"><button type="button" data-bs-toggle="modal" data-bs-target="#modalDiscussion"  wire:click="$emit('showDiscussion', {{ $item->id }})" class="btn btn-info text-decoration-none text-dark">{{$item->user->user_login}}</a></li>
    @endforeach
</ul>
@endpush